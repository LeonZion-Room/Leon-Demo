...
def show_fullscreen_image_and_open_url(image_path_or_url: str, target_url: str, delay_ms: int = 3000) -> None:
    """在已有 PySide6 应用中安全显示启动图窗口。

    - 若当前无 QApplication，则创建并启动事件循环；
    - 若已有 QApplication（主程序场景），仅调度显示，不重复启动事件循环。
    """
    ...
    QTimer.singleShot(max(0, int(delay_ms)), win.show)
