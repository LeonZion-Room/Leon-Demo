...
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QPoint, QSize, QUrl
...
class PDFMergeWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        # 主体：左文件列表 + 右参数与操作
        row = QHBoxLayout()
        row.setSpacing(dp(self.scale, 8))
        ...
        # 操作区
        act_row = QHBoxLayout()
        act_row.setSpacing(dp(self.scale, 6))
        ...
        act_row.addWidget(self.btn_merge)
        act_row.addWidget(self.btn_open)
        act_row.addWidget(btn_contact)
