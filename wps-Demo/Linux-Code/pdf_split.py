...
class PDFSplitWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        file_ops.setSpacing(dp(self.scale, 6))
        ...
        file_ops.addWidget(self.btn_pick_pdf)
        
        self.btn_clear_pdf = QPushButton("清空文件")
        self.btn_clear_pdf.setMinimumHeight(dp(self.scale, 72))
        self.btn_clear_pdf.clicked.connect(self.on_clear_pdf)
        file_ops.addWidget(self.btn_clear_pdf)
        
        right.addLayout(file_ops)
        ...
        # 添加拆分点列表标题
        lbl_points_title = QLabel("已选中截取页面号")
        lbl_points_title.setObjectName("section_title")  # 使用已有样式
        right.addWidget(lbl_points_title)
        
        self.points_list = QListWidget()
        self.points_list.setSelectionMode(QListWidget.SingleSelection)  # 确保可以选择项目
    ...
    def on_clear_pdf(self):
        """清空选中的PDF文件"""
        self.pdf_path = ""
        try:
            self.pdf_path_edit.setText("")
        except Exception:
            pass
        self.total_pages = 0
        self.current_page = 0
        self.split_points.clear()
        self.points_list.clear()
        self.status_label.setText("未选择PDF文件")
        self._update_preview()  # 更新预览
        self.btn_pick_pdf.setText("上传文件")
        # 保留输出路径不变，但可以选择清空
        self._build_thumbnails()  # 重建缩略图（会清空当前内容）
