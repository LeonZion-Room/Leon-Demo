...
def convert_pdf_to_docx(
    ...
    if docx_path is None:
        docx_path = suggest_docx_path(pdf_path)

    # 检查是否为Linux环境
    import platform
    if platform.system() != "Windows":
        raise RuntimeError(
            "PDF转DOCX功能当前仅支持Windows系统，需要Microsoft Word和COM接口支持。"
            "\nLinux环境下建议使用其他工具如LibreOffice进行转换。"
        )
    ...
    if core_pdf2docx is None:
        try:
            ...
            raise ImportError(
                f"未找到 before/pdf_fc.py 或导入失败，请确认项目结构和依赖。错误信息: {str(e)}"
            )
    ...
    def on_open_docx(self):
        ...
        try:
            ...
            if sys.platform == 'win32':
                ...
                subprocess.run(['open', self.docx_path], check=True)
            else:  # Linux
                subprocess.run(['xdg-open', self.docx_path], check=True)
        except Exception as e:
